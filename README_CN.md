...
灵感来自 [Andrej Karpathy 的 LLM-wiki 理念](https://x.com/karpathy)。纯 markdown 文件，零后台进程，为 AI agent 消费信息的方式而设计。
...
- **苏格拉底式学习。** `/distill-learn` 不只是保存你说的话，而是挑战你的理解、测试跨领域迁移、只有证明真正掌握后才存入。
...
三层挑战模式。用你的 brain 作为瞄准方向，但从你的知识之外拿弹药。第 1 层：从你已有条目中找内部矛盾。第 2 层：攻击盲区（brain 里缺什么）。第 3 层：引入你知识图谱之外的全新视角。不迎合——挑战者必须表态，并说明什么证据能改变立场。经受住考验的想法标记 `confidence_source: challenged`。
