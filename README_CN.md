...
- **Karpathy 三层架构。** 原始材料层（不可变证据）+ Wiki 层（你处理后的知识）+ Schema 层（brain 的运行规则）。三个共存层，继承自 Karpathy 的 LLM-wiki。
- **两层知识（inbox + domains）。** inbox/ 是 LLM 自动维护的 wiki——自动捕获一切，不丢东西。domains/ 是你确认过的正式知识——只有通过 `/distill`、`/distill-learn`、`/distill-challenge` 验证的条目才能进入。就像你脑中"知道的东西"和"验证过的经验"的区别。
...
**两层知识。** `inbox/` 是 LLM 自动维护的 wiki（Karpathy 的速度——不丢东西）。`domains/` 是你确认过的正式知识（只有通过 `/distill`、`/distill-learn`、`/distill-challenge` 验证的条目才能进入）。质量需要人看——和 Karpathy 一样，但你只需要看没经过学习或挑战验证的部分。
...
**AI 捕获，人决定。** inbox/ 自动积累（Karpathy 的复利机制）。但 domains/ 只接受你通过提取、学习或挑战验证过的知识。质量需要人看——没有捷径。
